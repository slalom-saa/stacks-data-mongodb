...
function Ensure-Current($path) {
    $JObject = [Newtonsoft.Json.JsonConvert]::DeserializeObject((Get-Content $path))
    #Ensure-Node -root $JObject -names 'Stacks.Logging.Serilog.MinimumLevel'
	#Ensure-Node -root $JObject -names 'Stacks.Logging.Serilog.UseLiterate'
    
    if ($JObject -ne $null) {
		"log  : Ensuring MongoDB Data settings "
        [Newtonsoft.Json.JsonConvert]::SerializeObject($JObject) > $path
    }
}
